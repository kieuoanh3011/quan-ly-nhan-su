...
public class PanelSlide extends javax.swing.JLayeredPane {
        ...
        login.addEventLogin(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent ae) {
                if (!animator.isRunning()) {
                    ...
                    if (login.checkUser()) {
                        showSlide(true);
                        login(login.getUserName(), login.getPass());
                    }
                }
            }
        });

        login.addEventExit(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                boolean check = MsgBox.confirm(login, "Bạn thực sự muốn thoát");
                if (check) {
                    System.exit(0);
                }
    ...
    private void login(String manv, String pass) {
        th = new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    if (nv == null) {
                        loading.showError("Sai mã nhân viên");
                    } else {
                        if (!nv.getMKTK().equals(pass)) {
                            loading.showError("Sai mật khẩu");
                        } else {
                            loading.doneLogin(nv);
                        }
